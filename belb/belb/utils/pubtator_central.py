...
class PubTatorDB:
    ...
    def __init__(
        ...
        self.schema = {
            "pmcid_to_pmid": "pmcid_pmid",
            "abstracts": "pubt_articles",
            "annotations": "pubt_annotations"
        }
    ...
    def init_database(self):
        if self.connection is None:
            raise RuntimeError("Database not opened yet")

        cursor = self.connection.cursor()

        # Table pmcid_pmid
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS pmcid_pmid (
            pmcid INTEGER,
            pmid INTEGER
        )
        """)
        ...
        # Table pubt_articles
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS pubt_articles (
        ...
        # Table pubt_annotations
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS pubt_annotations (
            pmid TEXT,
            type TEXT,
            text TEXT,
            start INTEGER,
    ...
    def _get_query_annotations(self, ids: set[str], entity_types: list) -> str:
        ...
        types_in = "', '".join(entity_types)
        query =  (f"SELECT pmid,type,text,start,end "
                  f"FROM pubt_annotations "
                  f"WHERE pmid IN {tuple(ids)} AND lower(type) IN ('{types_in}') "
                  f"ORDER BY pmid")
        """
        return query
